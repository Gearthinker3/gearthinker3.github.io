...
function checkGameResult() {
    ...
    setTimeout(() => {
        ...
        // ✅ Save game result in log
        currentLog.result = resultMsg;
        currentLog.dealerCards = dealerHand;
...
function exportLogsToCSV(logs) {
    ...
    const rows = logs.map(log => {
        return [
            ...
            log.handId,
            log.actions.join("|"),
            log.playerCards.join("|"),
            log.dealerCards.join("|"),
            `"${log.result}"`, // wrap in quotes in case of commas
            log.timestamp
        ].join(",");
    });
    ...
    link.setAttribute("download", "blackjack_game_logs.csv");
